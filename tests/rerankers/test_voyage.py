...
"""Tests for VoyageReranker — respx-mocked HTTP, no real API calls."""
...
from cenote.rerankers import VoyageReranker
from cenote.rerankers.voyage import VOYAGE_RERANK_URL
...
class TestVoyageReranker:
    ...
    async def test_model_id_format(self) -> None:
        r = VoyageReranker(api_key="k", model="rerank-2")
        assert r.model_id == "voyage:rerank-2"

    @respx.mock
    async def test_reorders_by_relevance(self) -> None:
        respx.post(VOYAGE_RERANK_URL).mock(
        ...
        assert out[0].retriever == "vector+rerank:voyage"
    ...
    async def test_top_k_caps_output(self) -> None:
        respx.post(VOYAGE_RERANK_URL).mock(
            return_value=httpx.Response(
                200,
                json={"data": [{"index": i, "relevance_score": (10 - i) / 10} for i in range(10)]},
            )
        )
        reranker = VoyageReranker(api_key="k")
    ...
    async def test_top_k_none_returns_all(self) -> None:
        respx.post(VOYAGE_RERANK_URL).mock(
            return_value=httpx.Response(
                200,
                json={"data": [{"index": i, "relevance_score": (5 - i) / 5} for i in range(5)]},
            )
        )
        reranker = VoyageReranker(api_key="k")
    ...
    async def test_retries_on_5xx_then_succeeds(self) -> None:
        route = respx.post(VOYAGE_RERANK_URL).mock(
            side_effect=[
                httpx.Response(503),
                httpx.Response(
                    200,
        ...
        reranker = VoyageReranker(api_key="k", max_retries=2, base_backoff_seconds=0.0)
    ...
    async def test_raises_rate_limit_after_retries(self) -> None:
        respx.post(VOYAGE_RERANK_URL).mock(return_value=httpx.Response(429))
        reranker = VoyageReranker(api_key="k", max_retries=1, base_backoff_seconds=0.0)
    ...
    async def test_authorization_header(self) -> None:
        ...
        reranker = VoyageReranker(api_key="secret-token")
        await reranker.rerank("q", [make_result("x", 0.0)])
        assert route.calls.last.request.headers["authorization"] == "Bearer secret-token"

    @respx.mock
    async def test_empty_results_short_circuits(self) -> None:
        route = respx.post(VOYAGE_RERANK_URL)
        reranker = VoyageReranker(api_key="k")
        assert await reranker.rerank("q", []) == []
        assert route.call_count == 0

    @respx.mock
    async def test_batches_when_exceeding_max(self) -> None:
        ...
        third = httpx.Response(
            200,
            json={"data": [{"index": 0, "relevance_score": 0.7}]},
        )
        route = respx.post(VOYAGE_RERANK_URL).mock(side_effect=[first, second, third])
        reranker = VoyageReranker(api_key="k", batch_size=2)
        inputs = [make_result(f"t{i}", 0.0, idx=i) for i in range(5)]
        out = await reranker.rerank("q", inputs)
        assert route.call_count == 3
        assert len(out) == 5
        assert [r.score for r in out] == pytest.approx([0.9, 0.8, 0.7, 0.2, 0.1])
